...
class GaussianModel:
    ...
    def gaussian_init(self):
        ...
        fused_point_cloud = (fused_point_cloud*2)  # match scene scale (~±2m)
        self.spatial_lr_scale = 1.0  # enable xyz position updates
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, radii, max_gaussians=0):
        ...
        if max_gaussians and max_gaussians > 0:
            ...
            if n > max_gaussians:
                excess = n - max_gaussians
                opacities = self.get_opacity.squeeze()
                _, prune_indices = opacities.topk(excess, largest=False)
                cap_mask = torch.zeros(n, dtype=torch.bool, device='cuda')
                cap_mask[prune_indices] = True
                self.prune_points(cap_mask)
    ...
    def dist_signal_mapping(self, tx, r_o):
        if isinstance(r_o, (list, tuple)):
            r_o = torch.tensor(r_o, dtype=torch.float32)
        r_o = r_o.cuda().squeeze()   # ensure [3] on GPU

        # squeeze batch dim from dataloader if present
        tx = tx.squeeze(0) if tx.dim() == 2 else tx  # [3]

        pts = self._xyz                              # [N, 3]
        view = F.normalize(pts - tx.unsqueeze(0).expand(pts.size(0), -1), dim=-1)  # [N, 3]
        N = pts.size(0)
        tx_exp = tx.unsqueeze(0).expand(N, -1)       # [N, 3]
        rx_exp = r_o.unsqueeze(0).expand(N, -1)      # [N, 3]

        raw = self.network_fn(pts, view, tx_exp, rx_exp)   # [N, 4]

        # per-Gaussian path distance from RX (fixes original scalar norm bug)
        path = torch.norm(pts - r_o, dim=-1, keepdim=True)  # [N, 1]
        path_loss = 0.025 / path.clamp(min=1e-6)            # [N, 1]

        att_a, att_p, s_a, s_p = raw[...,0], raw[...,1], raw[...,2], raw[...,3]
        att_p = torch.sigmoid(att_p) * np.pi * 2
        s_p   = torch.sigmoid(s_p)   * np.pi * 2
        att_a = abs(F.leaky_relu(att_a))
        s_a   = abs(F.leaky_relu(s_a))
...
class MappingNetwork(nn.Module):
    ...
    def __init__(self, D=8, W=128, skips=[4],
                 input_dims={'pts':3, 'view':3, 'tx':3, 'rx':3},
                 multires = {'pts':10, 'view':10, 'tx':10, 'rx':10},
                 is_embeded={'pts':True, 'view':True, 'tx':True, 'rx':True},
        ...
        self.embed_rx_fn, input_rx_dim = get_embedder(multires['rx'], is_embeded['rx'], input_dims['rx'])

        ## attenuation network — takes only point positions (unchanged)
        ...
        ## signal network — extended with rx encoding
        self.signal_linears = nn.ModuleList(
            [nn.Linear(input_view_dim + input_tx_dim + input_rx_dim + W, W)] +
            [nn.Linear(W, W//2)]
        )
    ...
    def forward(self, pts, view, tx, rx):
        ...
        x = torch.cat([feature, view, tx, rx], -1)
